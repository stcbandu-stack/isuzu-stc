...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    const systemPrompt = `
คุณเป็นผู้เชี่ยวชาญด้านการวิเคราะห์ปัญหาเคสสินเชื่อรถยนต์
ด้านล่างคือข้อมูลสรุปจาก Google Sheet: แต่ละสถานะ (case_status) มีจำนวนเคสและตัวอย่าง case_note (โน้ตปัญหา) หลายรายการ
...
รูปแบบผลลัพธ์ที่ต้องการ (ตัวอย่างโครงสร้าง):

1) รอเอกสารครบ (จำนวน 12 เคส)
- ปัญหาหลัก: ลูกค้าส่งสำเนาบัตร/สลิปเงินเดือนไม่ครบ หรือส่งช้า
- ประเด็นที่เจอบ่อย: ...
- แนวทางที่ควรทำ: ...

2) รอเซ็นสัญญา (จำนวน 8 เคส)
- ...
...
ให้ตอบกลับเป็น "ข้อความสรุป" อย่างเดียว ไม่ต้องแทรก JSON หรือโค้ด
    ...
    const result = await ai.models.generateContent({
      ...
      contents: [
        { role: 'user', parts: [{ text: systemPrompt }] },
        {
          ...
          parts: [
            {
              text: JSON.stringify(statusArray, null, 2),
            },
          ],
        },
      ],
    ...
    console.error('win-finance-status-summary error:', error);
    return new Response(
      JSON.stringify({
        success: false,
