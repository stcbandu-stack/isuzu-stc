...
// Spreadsheet ID for New Car Online Tracking 2569
const SPREADSHEET_ID = '1PcJKn26gvipZvLtB5fLBqTnuL9cDha8GO9e2iNkBI0A';
...
export interface NewCarOnlineRecord {
  row_num: number;
...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    const records: NewCarOnlineRecord[] = [];
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const record: NewCarOnlineRecord = {
        row_num: i + 1,
    ...
    console.error('Error fetching New Car Online Tracking data:', error);
    return new Response(JSON.stringify({ 
      success: false,
